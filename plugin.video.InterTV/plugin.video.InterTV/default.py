...
addon = xbmcaddon.Addon('plugin.video.InterTV')
...
__datapath__ = path
__addonname__ = "plugin.video.InterTV"

if os.path.exists(favorites)==True:
    FAV = open(favorites).read()
else: FAV = []

SOURCES = [["ventas", "http://tvmasplus.com/sales.html"]]
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.tv-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(InterTV,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(InterTV,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def getSources():
        if os.path.exists(favorites) == True:
            ...
            if FAV == "[]":
                ...
                addDir('[COLOR cyan][B]- Favorites Channels InterTV [/COLOR][/B]','url',4,os.path.join(home, 'resources', 'favorite.png'),FANART,'','','','')
                addDir('','',100,'',FANART,'','','','')
...
def urlsolver(url):
    ...
    if host:
    	...
    	xbmc.executebuiltin("XBMC.Notification(InterTV,Urlresolver donot support this domain. - ,5000)")
    return resolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('InterTV','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from InterTV Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
            if not name in FAV:
                contextMenu.append(('Add to InterTV Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(InterTV,No IMDB match found ,7000,"+icon+")")
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie="",allinfo={}):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from InterTV Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to InterTV Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
def LOGIN(username,password,hidesuccess):
	if username != '':
		...
		lc = username.lower()	
		logged_in = weblogin.doLogin(path,path,username,password)
	else:
		Notify('Login Failure','Please fill username and password in the add-on setting','4000',False)
		exit(3);
	...
	if logged_in == True:
		...
		Notify('Welcome back '+uc,'InterTV','4000',False)
		...
		Notify('Login Failure',uc+' could not login','4000',False)
		exit(3);
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(InterTV,Failed to extract regex. - "+"this"+",4000,"+icon+")")
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(InterTV,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
